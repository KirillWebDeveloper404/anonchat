...
class dbworker:
    ...
    def get_random_number_user(self, telegram_id):
        ''' Получить информацию о случайном числе очереди пользователя '''
        with self.connection:
            result = self.cursor.execute('SELECT `randomQueueNumber` FROM `users` WHERE `telegram_id` = ?',(telegram_id,)).fetchone()
            return result

    def update_random_number_user(self,telegram_id):
        ''' Обновить информацию о случайном числе очереди пользователя '''
        with self.connection:
            self.cursor.execute('UPDATE `users` SET `randomQueueNumber` = ? WHERE `telegram_id` = ?', (random.randint(0, 9999999), telegram_id))
